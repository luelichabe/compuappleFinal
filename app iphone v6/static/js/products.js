...
function buscarProducto() {
    ...
    const inputValue = input.value.toLowerCase(); // Guarda el valor del input antes de vaciarlo

    productos.forEach(producto => {
        const nombreProducto = producto.querySelector('.card-title').textContent.toLowerCase();
        if (nombreProducto.includes(inputValue)) {
    ...
    // Muestra u oculta el mensaje según si hay productos o no
    if (encontrados) {
        noResultados.style.display = 'none'; // Oculta el mensaje si hay productos
        noResultados.classList.remove('mostrar'); // Remueve la clase si estaba
    } else {
        noResultados.style.display = 'block'; // Muestra el mensaje si no hay productos
        noResultados.classList.add('mostrar'); // Añade la clase 'mostrar'

        // Espera brevemente antes de añadir el listener para evitar que se oculte de inmediato
        setTimeout(() => {
            document.addEventListener('click', ocultarMensaje, { once: true });
        }, 0);
    }

    input.value = ""; // Vacía el campo de búsqueda después de buscar
}

function ocultarMensaje(event) {
    const noResultados = document.getElementById('no-resultados');
    if (noResultados.classList.contains('mostrar')) {
        noResultados.style.display = 'none'; // Oculta el mensaje
        noResultados.classList.remove('mostrar'); // Remueve la clase 'mostrar'
    }
}
...
async function checkout() {
    ...
    if (response.ok) {
        ...
        if (data.success) {
            ...
            // Reinicia el carrito después de finalizar la compra
            cartCount = 0; // Reinicia el contador interno a cero
            cartCounter.textContent = "0"; // Reinicia el contador visual en el icono del carrito

            // Opcional: Vaciar la vista del carrito
...
//sincoronizar carrito al cargar la pagina 
document.addEventListener("DOMContentLoaded", async function () {
    // Sincronizar el contador con el estado actual del carrito en el servidor
    try {
        const response = await fetch('/get-cart-count'); // Endpoint para obtener la cantidad actual de productos en el carrito
        if (response.ok) {
            const data = await response.json();
            if (data.success) {
                cartCount = data.count; // Actualiza el contador interno
                cartCounter.textContent = cartCount; // Actualiza el contador visual
            }
        } else {
            console.error("Error al obtener la cantidad de productos en el carrito");
        }
    } catch (error) {
        console.error("Error en la solicitud:", error);
    }
